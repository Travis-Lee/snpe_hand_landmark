...
class Config(object):
    def __init__(self, cfg_file, cfg_from_json, outputbasedir, devicelist, hostname, deviceostype, userbuffer_mode, perfprofile, profilinglevel, enable_init_caching):
        ...
        if deviceostype == CONFIG_DEVICEOSTYPES_LE:
            ...
            self._stl_library = None
            self._artifacts_config = load_json(os.path.join(config_prefix, "snpebm", SNPE_BENCH_LE_OE_GCC64_HF_ARTIFACTS_JSON))
        elif deviceostype == CONFIG_DEVICEOSTYPES_LE64_OE_GCC64:
            self._architectures = [ARCH_AARCH64]
            self._platform_os = PLATFORM_OS_LINUX
            self._compiler = COMPILER_GCC64
            self._stl_library = None
            self._artifacts_config = load_json(os.path.join(config_prefix, "snpebm", SNPE_BENCH_LE64_OE_GCC64_ARTIFACTS_JSON))
    ...
    def __quick_verify__(self):
        try:
            ...
            for _key in CONFIG_JSON_ROOTKEYS:
                # check optional keys
                if not _key is CONFIG_PERF_PROFILE_KEY and not _key is CONFIG_CPU_FALLBACK_KEY and not _key is CONFIG_HOST_NAME_KEY and not _key is CONFIG_BUFFERTYPES_KEY and not _key is CONFIG_PROFILING_LEVEL_KEY and not _key is CONFIG_USERBUFFER_FLOAT_OUTPUT_KEY:
