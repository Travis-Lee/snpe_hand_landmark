...
uint32_t ${op_name}StaticParamsNum = ${len(operator.tensor_param) + len(operator.scalar_param)};
uint32_t ${op_name}InputsNum = ${len(operator.input)};
uint32_t ${op_name}OutputsNum = ${len(operator.output)};
SnpeUdo_QuantizationType_t ${op_name}InputQuantizationTypes [] = {${','.join(input.quant_type for input in operator.input)}};
