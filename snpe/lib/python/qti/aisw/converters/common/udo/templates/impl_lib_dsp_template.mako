...
#include "${operator.name}ImplLib${runtime.title()}.h"
%endif
%endfor
...
char opTypes [] = "${' '.join(operator.name for operator in operators)}";
 ...
 * @param implementationInfo i.e. {SNPE_UDO_CORETYPE_DSP, "${package_name}", "${' '.join(operator.name for operator in operators)}", 1}
 * @return error type - no error
 */
SnpeUdo_ErrorType_t
...
%if core_type in operator.core_types:
    ...
    UdoDspShared *${operator.name}Op  = new_${operator.name}(infra);
    if (${operator.name}Op == NULL)
    {
        return SNPE_UDO_INVALID_ARGUMENT;
    }
    registerOpToOpList("${operator.name}", ${operator.name}Op);
