...
#include "${operator.name}ImplLib${runtime}.hpp"
%endif
%endfor
...
SnpeUdo_ErrorType_t SnpeUdo_initImplLibrary(void* globalInfrastructure)
{
    ...
    UDO_VALIDATE_RETURN_STATUS(ImplLib.registerOpDefinition
                               ("${operator.name}",
                               std::unique_ptr<${_runtime_formatted_op_name(operator.name)}OpDef>(new ${_runtime_formatted_op_name(operator.name)}OpDef("${operator.name}",${len(operator.input)}, ${len(operator.output)}, gpuGlobalInfrastructure))))
    %else:
    UDO_VALIDATE_RETURN_STATUS(ImplLib.registerOpDefinition
                               ("${operator.name}",
                               std::unique_ptr<${_runtime_formatted_op_name(operator.name)}OpDef>(new ${_runtime_formatted_op_name(operator.name)}OpDef("${operator.name}",${len(operator.input)}, ${len(operator.output)}))))
    %endif
    %endif
    %endfor
