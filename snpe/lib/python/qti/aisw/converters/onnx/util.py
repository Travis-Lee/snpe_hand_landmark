...
from qti.aisw.converters.common.utils.translation_utils import IRPaddingStrategies, broadcastable
...
def set_to_weights_and_biases(src_op, graph, mode='bias'):
    ...
    if len(const_input):
        ...
        if mode == 'bias':
            ...
            if hasattr(prev_.op, 'weights') or \
                    (hasattr(prev_.op, 'bias') and prev_.op.bias.shape == const_value.shape):
                bias = const_value
                return actual_inputs, weights, bias
        elif mode == 'weights':
            # if the previous op has a bias input and weights that are broadcastable with each other,
            # then we know the weights can be absorbed without changing the final output.
            if (hasattr(prev_.op, 'bias') and broadcastable(prev_.op.bias.shape, const_value.shape)) and \
               (hasattr(prev_.op, 'weights') and broadcastable(prev_.op.weights.shape, const_value.shape)):
                weights = const_value
                return actual_inputs, weights, bias
    ...
    def __init__(self, model):
        ...
        for tensor in model.graph.initializer:
            ...
            if not tensor.dims:
                ...
                # since unable to modify tensor.dims, reshaping again to have numpy array proper config
                onnx_tensor = onnx_tensor.reshape(1)
            self.weight_map[str(tensor.name)] = WeightData(onnx_tensor)
