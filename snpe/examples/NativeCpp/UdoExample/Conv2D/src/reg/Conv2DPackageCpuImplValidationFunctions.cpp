...
#include "Conv2DPackageCpuImplValidationFunctions.hpp"
#include <string.h>
using namespace UdoUtil;

SnpeUdo_ErrorType_t
ConvolutionCpuValidationFunction::validateOperation(SnpeUdo_OpDefinition_t* def) {
    SnpeUdo_ErrorType_t status = SNPE_UDO_NO_ERROR;

    if (def == nullptr)
        return SNPE_UDO_INVALID_ARGUMENT;

    if (strcmp(def->operationType, "Convolution"))
