...
bool
initUdoRegInfoStruct(std::unique_ptr<SnpeUdo_RegInfo_t> &regInfoStruct,
...
UdoOperationInfo::copyOperationInfo(SnpeUdo_OperationInfo_t* operationInfoPtr) {
    ...
    auto operationStatus = copyOperationParams(operationInfoPtr);
    auto coreStatus = copyOperationCoreInfo(operationInfoPtr);

    UDO_VALIDATE_MSG(operationStatus != SNPE_UDO_NO_ERROR,
                 operationStatus,
                 "Attempt to copy operation params failed")

    UDO_VALIDATE_MSG(coreStatus != SNPE_UDO_NO_ERROR,
                 operationStatus,
                 "Attempt to copy operation core types failed")
